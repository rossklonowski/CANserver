...
void Custom_Adafruit_SSD1325::setup(int _mosi, int _clk, int _dc, int _rst, int _cs) {
  sid = _mosi;
  sclk = _clk;
  dc = _dc;
  rst = _rst;
...
void Custom_Adafruit_SSD1325::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
void Custom_Adafruit_SSD1325::begin(void) {
  // set pin directions
  if (sclk != -1) {
    pinMode(sid, OUTPUT);
    pinMode(sclk, OUTPUT);
...
void Custom_Adafruit_SSD1325::invertDisplay(uint8_t i) {
  if (i) {
...
void Custom_Adafruit_SSD1325::command(uint8_t c) {
...
void Custom_Adafruit_SSD1325::data(uint8_t c) {
  digitalWrite(cs, HIGH);
...
void Custom_Adafruit_SSD1325::display(void) {
...
// clear everything
void Custom_Adafruit_SSD1325::clearDisplay(void) {
  memset(buffer, 0, (SSD1325_LCDWIDTH * SSD1325_LCDHEIGHT / 8));
}

void Custom_Adafruit_SSD1325::spixfer(uint8_t x) {
  if (sclk == -1) {
    SPI.transfer(x);
    return;
  }
